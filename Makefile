...
DEPF       := .depend
#Adel IGN        := --ignore netcdf --ignore grib_api #Hanieh
...
-include $(ROOT)/Options#.daint_xc50
ifndef F90
...
paropt :
	@$(MAKE) -C $(OBJDIR) -f $(ROOT)/Makefile NUDGING=1 RTTOV7=1 GRIBDWD=1 NETCDF=1 MPI=1 OPT=1 depend info
	@$(MAKE) -C $(OBJDIR) -f $(ROOT)/Makefile NUDGING=1 RTTOV7=1 GRIBDWD=1 NETCDF=1 MPI=1 OPT=1 $(ROOT)/$(TARGET)

pardebug :
	@$(MAKE) -C $(OBJDIR) -f $(ROOT)/Makefile NUDGING=1 RTTOV7=1 GRIBDWD=1 NETCDF=1 MPI=1 DEBUG=1 depend info
	@$(MAKE) -C $(OBJDIR) -f $(ROOT)/Makefile NUDGING=1 RTTOV7=1 GRIBDWD=1 NETCDF=1 MPI=1 DEBUG=1 $(ROOT)/$(TARGET)
